...
#include <Arduino.h>
#include <errno.h>
#include <lwip/sockets.h>       // [驱动] ESP32 lwIP socket
#include <freertos/FreeRTOS.h>
#include <freertos/task.h>      // [驱动] FreeRTOS vTaskDelay
#endif
// ======================== 驱动层结束 ========================
...
#else
    int n = recv(fd_, buf + done, len - done, 0);  // [驱动] lwIP recv
    if (n > 0) { done += (size_t)n; start = millis(); continue; }
    if (n == 0) return false;
    if (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR) {
      if (millis() - start > 5000) return false;
      vTaskDelay(1);  // [驱动] 让出 CPU 给 WiFi 任务
      continue;
    }
#endif
    return false;
...
bool PacketCodec::writeExact(const uint8_t* buf, size_t len) {
  ...
  while (done < len) {
    ...
    if (chunk > 1460) chunk = 1460;  // MTU 分片
#ifdef _WIN32
...
#else
    int n = send(fd_, buf + done, chunk, MSG_NOSIGNAL);  // [驱动] lwIP send
    if (n > 0) { done += (size_t)n; start = millis(); continue; }
    if (n == 0) return false;
    if (errno == EAGAIN || errno == EWOULDBLOCK || errno == EINTR) {
      if (millis() - start > 5000) return false;
