...
MinecraftServer::MinecraftServer(uint16_t port)
    : network_(port), online_(0), max_players_(100) {
  for (uint8_t i = 0; i < kMaxClients; ++i) {
    clients_[i].fd = -1;
    clients_[i].used = false;
    clients_[i].state = STATE_NONE;
  }
  strncpy(description_, "ESP32MC Server", sizeof(description_) - 1);
}
